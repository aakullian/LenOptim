...
load("data/Len_optim_data_ZAF_extra_ages.RData")
...
df <- read_delim("data/LEN Quantification Perf Review 24.06.2025_With Geo-Coordinates 24.06.2024.csv",
                 delim = ",",
                 locale = locale(encoding = "Latin1"),
                 guess_max = 5000)  # ensures full type guessing

# Load district names
district_names <- read.csv('data/District Names.csv') %>% #names to merge facility to NAOMI datasets
  dplyr::select(area_id,District) %>%
  rename(district=District)

# Load the oral prep disagg data (use the correct sheet name)
df_raw <- read_excel("data/Disaggregated PrEP Initiations April 2020 to March 2025.xlsx", sheet = "Sheet2")
...
ndoh_len_plan = read.csv("data/Slide12_Target_Population_Lenacapavir_Corrected.csv")
...
save(incidence_df_quant, file="output/incidence_df_quantiles.RData")
...
save(ndoh_len_plan_plot, ndoh_len_plan_plot_scatter, file = "output/extra_plots.RData")
