...
allocate_prep_by_risk_with_stratified_prob <- function(facility_df,
  ...
  incidence_df <- incidence_df %>%
      filter(n_quantiles == quant_scenario_selection) %>%
      mutate(
        sex = tolower(sex),
...
p_district_facilities <- ggplot(district_map1) +
  ...
  p_district_pop <- ggplot(district_map3) +
    ...
    scale_fill_viridis_c(name = "% Len coverage", na.value = "grey90", limits=c(0,8)) +
    labs(
      title = "District-level Len coverage (Adults 15+)"
    ) +
...
generate_prep_allocation_outputs <- function(facility_df,
  ...
  # 4. Format map layout
  label_text <- paste0("Scenario: ", "Optimized district-level allocation (MF 15-49) | ",quant_scenario_selection, " risk-groups | ", units," Len person-years")
