...
#quant_scenario_yn = 1
units = 500000   # harmonised with Lise (Thembisa) Phase 1 (~500k person-years)
...
# Run all scenarios
source("02_model_district_priority.R", echo=F) #District simulations (running different degrees of geographic versus demographic targeting)
...
for (i in seq_len(nrow(param_grid))) {
  ...
  save(outputs,
       file = paste0("output/Len_optim_RSA_output_DISTRICT_prioritization_", scenario_tag, ".RData"))

  ggsave(paste0("output/Len_optim_RSA_output_DISTRICT_prioritization_", scenario_tag, ".jpg"),
