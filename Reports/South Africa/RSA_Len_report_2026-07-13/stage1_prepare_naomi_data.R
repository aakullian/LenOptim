...
# =============================================================================
# STAGE 1 (run once): build data/Len_optim_data_ZAF_extra_ages.RData from raw
# UNAIDS Naomi inputs. This is the upstream generator for the report pipeline
# (00_data_cleaning.R -> 01/02 engines -> Len_RSA_report.Rmd).
#
# SLOW: the within-district risk simulation loops over every district/age/sex.
# Only re-run if the raw Naomi inputs change; otherwise use the committed
# data/Len_optim_data_ZAF_extra_ages.RData.
#
# Reproducibility: the original had no seed, so results were not reproducible.
# set.seed() below fixes that. NOTE this means a fresh run will NOT bit-match the
# committed .RData behind the original report (that snapshot was seedless).
# =============================================================================
...
# Reproducible RNG for the gamma/bernoulli risk simulation below
set.seed(12345)

# Raw Naomi inputs live in <repo root>/NAOMI ; this folder is 3 levels down
naomi_dir <- "../../../NAOMI"

#Bring in Africa countries shp and clip to extent desired
africa_adm0 <- read_sf(file.path(naomi_dir, 'Africa_admin0/afr_g2014_2013_0.shp')) #shapefile
...
naomi_indicators <- readRDS(file.path(naomi_dir, 'Combined Subnational dataset/naomi3_2024_07_01.rds'))
naomi_indicators_adm0 <- readRDS(file.path(naomi_dir, 'Combined Subnational dataset/naomi1_2024_07_01.rds'))
naomi_shp <- read_sf(file.path(naomi_dir, 'Combined Subnational dataset/2024_ssa_lowest_level_35.geojson')) #shapefile
...
age_groups <- c("15-19","20-24","25-34","35-49", "50+","15-49") # "15-19","20-24","25-29","30-34","35-39","40-44","45-49"
...
for (i in seq(1,nrow(inc_list),)){
  for(shape in c(1)){
      ...
      for(quant in seq(0.125,0.875,0.25)){ #4 risk groups with 25% of the population in each group
      #for(quant in seq(0.125-0.0625,0.875+0.0625,0.125)){  #8 risk groups with 12.5% of the population in each group
      #for(quant in seq(0.05,0.95,0.1)){  #10 risk groups with 12.5% of the population in each group
      sub_pop <- pop %>%
...
#FINER %TILE BINS: simulate incidence by district/age/sex across empirical risk groups estimated from a gamma distribution of continual risk
efficacy=0.95
n_quantiles = c(1)
n_quantiles = c(2,4,10,20)
n=1
...
for(n_quantiles in n_quantiles){
  for (i in seq(1,nrow(inc_list))){
    ...
    total_pop = length(x)
    pop$quantile = rank(pop$x)/nrow(pop)
    for(quant in seq(0, (1/n_quantiles)*(n_quantiles-1), 1/n_quantiles)){ #n_quantiles risk groups with 1/n_quantiles% of the population in each group
      #for(quant in seq(0.125-0.0625,0.875+0.0625,0.125)){  #8 risk groups with 12.5% of the population in each group
      #for(quant in seq(0.05,0.95,0.1)){  #10 risk groups with 12.5% of the population in each group
      sub_pop <- pop %>%
        filter(quantile>quant, quantile < quant+(1/n_quantiles)) %>% 
        summarise(
          sex=sex,
          age_group_label=age_group_label,
          iso3=iso3,
          ...
          pt=(tx+daly*cdt)/nnt,
          shape=shape, 
          quant_target=quant+1/n_quantiles,
          n_quantiles=n_quantiles) %>% 
        mutate( #This is just run for a scenario with no quantiles (ie == 1) so that incidence in the sample is the same as that in the district
          inc_in_sample = if_else(n_quantiles == 1,
                                  inc * 1000,
                                  inc_in_sample),
          total_infected_subsample = if_else(n_quantiles == 1,
                                            total_infections_district_age_sex,
                                            total_infected_subsample),  # 
          infections_averted = total_infected_subsample * efficacy
        ) 
      df1=sub_pop
      if(n==1){df2<-df1}else(df2<-rbind(df1,df2))
      n=2
      print(paste("working on inc=",inc,"shape=" ,shape,"quantile=",quant,"sex=",sex,"age group=",age_group_label, "n_quantiles=", n_quantiles))
      }
    }
  }
}
...
risk_dist_targeting_fine_scale_FULL_PERCENTILES = df2
risk_dist_targeting_fine_scale_FULL_PERCENTILES$inc_mult = risk_dist_targeting_fine_scale_FULL_PERCENTILES$inc_in_sample/risk_dist_targeting_fine_scale_FULL_PERCENTILES$inc_district #column of the relative difference in incidence between the targeted pop and the general pop = risk_dist_targeting_fine_scale %>%


## Save select dataframes ##
save(naomi_ssa_shp_m, risk_dist_targeting_fine_scale, risk_dist_targeting_fine_scale_FULL_PERCENTILES, file = "data/Len_optim_data_ZAF_extra_ages.RData")
