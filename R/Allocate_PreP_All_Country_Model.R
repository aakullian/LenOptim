...
# 3. % of population covered by PrEP by district
  ...
  p_district_pop <- ggplot(district_map2) +
    ...
    scale_fill_viridis_c(name = "% Catchment Pop Covered", na.value = "grey90", limits=c(0,max(district_map2$percent_pop_covered, na.rm=T)+1)) +
    labs(
  ...
  p_district_inf_averted <- ggplot(district_map3) +
    ...
    scale_fill_viridis_c(name = "Infections averted (%)", na.value = "grey90", limits=c(0,max(district_map3$inc_reduction, na.rm=T)+1)) +
    labs(
