...
map_prep_allocation_scenario <- function(results_list,
  ...
  p_district_pop <- ggplot(district_map2) +
    ...
    scale_fill_viridis_c(name = "% PrEP coverage", na.value = "grey90") +
    labs(
...
generate_prep_allocation_outputs <- function(facility_df,
  ...
  # 4. Format map layout
  label_text <- paste0("Scenario: $", selected_cost, "/unit, $", format(selected_budget/1000000, big.mark = ","),"M",  " budget ",", " ,"min facility capacity: " ,100, " initiations")
