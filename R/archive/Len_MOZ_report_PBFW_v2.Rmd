...
assumption_table %>%
  mutate(Value = ifelse(Parameter == 
    "HIV negative pregnant women population (Gaza and Zambezia)",
    comma(as.numeric(Value)),  # adds commas, e.g., 528,995
    Value)) %>%
...
```{r results-text, results='asis', echo=FALSE, message=FALSE, warning=FALSE}
...
# ---- 1) Derive tidy fields from the raw numeric summary ----

cs <- combined_summary %>%
  mutate(
    target_population = case_when(
      str_detect(scenario, "output_PBFW_prioritization") ~ "PBFW only",
      str_detect(scenario, "output_NON_PBFW_prioritization") ~ "Non PBFW",
      str_detect(scenario, "GENPOP") ~ "Non PBFW",
      TRUE ~ "Uncategorized"
    ),
    percent_target = str_extract(scenario, "(?<=_)\\d+$") %>% as.numeric()
  )
# Helper to return (min, max) safely
...
gp   <- cs %>% filter(target_population == "Non PBFW")
pbfw <- cs %>% filter(target_population == "PBFW only")

# ---- 2) Compute ranges and summaries for GP and PBFW ----

gp_inf    <- rng(gp$infections_averted)
gp_inc    <- rng(gp$percent_reduction_in_incidence)
gp_costIA <- rng(gp$cost_per_infection_averted)
gp_nnt    <- rng(gp$number_needed_to_treat)
gp_dists  <- rng(gp$facilities_with_allocation)
gp_cov    <- rng(gp$percent_target)
# gp_share  <- rng(gp$share_15_24)
gp_inc_alloc <- rng(gp$avg_incidence_allocated)

pbfw_inf    <- rng(pbfw$infections_averted)
pbfw_inc    <- rng(pbfw$percent_reduction_in_incidence)
pbfw_costIA <- rng(pbfw$cost_per_infection_averted)
pbfw_nnt    <- rng(pbfw$number_needed_to_treat)
pbfw_dists  <- rng(pbfw$facilities_with_allocation)
pbfw_cov    <- rng(pbfw$percent_target)
#pbfw_share  <- rng(pbfw$share_15_24)
pbfw_inc_alloc <- rng(pbfw$avg_incidence_allocated)

# ---- 3) Vertical transmission share among PBFW (infant share of total) ----

pbfw_infant_total <- if ("infant_infections_averted" %in% names(pbfw)) {
sum(pbfw$infant_infections_averted, na.rm = TRUE)
} else {

# Requires rho and k defined earlier in your analysis

stopifnot(exists("rho"), exists("k"))
sum(pbfw$infections_averted * (rho * k) / (1 + rho * k), na.rm = TRUE)
}
pbfw_total_inf <- sum(pbfw$infections_averted, na.rm = TRUE)
pbfw_infant_share_pct <- 100 * pbfw_infant_total / pbfw_total_inf

# ---- 4) Compose the narrative with glue ----

text_block <- glue(
"A general population allocation strategy would avert between ",
"{comma(gp_inf[1])}–{comma(gp_inf[2])} infections over 1 year ",
"({round(gp_inc[1],1)}–{round(gp_inc[2],1)}% incidence reduction) ",
"depending on uptake assumptions ({gp_cov[1]}%–{gp_cov[2]}% of the identified population at risk). ",
"Prioritizing Lenacapavir to PBFW would avert between ",
"{comma(pbfw_inf[1])}–{comma(pbfw_inf[2])} infections over 1 year ",
"({round(pbfw_inc[1],1)}–{round(pbfw_inc[2],1)}% incidence reduction), ",
"approximately twice the number of infections averted compared to a general population strategy. ",
"Vertical transmissions accounted for {round(pbfw_infant_share_pct)}% of the total infections averted in the PBFW scenarios. ",
"The cost per infection averted was lower when prioritizing PBFW ($",
"{comma(round(pbfw_costIA[1]))}–{comma(round(pbfw_costIA[2]))} per infection averted) ",
"compared to a general population strategy ($",
"{comma(round(gp_costIA[1]))}–{comma(round(gp_costIA[2]))}). ",
"The number needed to treat (NNT) to avert one infection was also lower when prioritizing PBFW (",
"{round(pbfw_nnt[1])}–{round(pbfw_nnt[2])}) compared to a general population strategy (",
"{round(gp_nnt[1])}–{round(gp_nnt[2])}). ",
"Across scenarios, between {pbfw_dists[1]}–{pbfw_dists[2]} districts would receive Lenacapavir, ",
"with higher uptake scenarios resulting in fewer districts receiving Lenacapavir. ",
"In all scenarios, 70-80% of Lenacapavir was allocated to women 15–24 years of age ",
"and the average incidence in the population receiving Lenacapavir ranged from ",
"{round(gp_inc_alloc[1],1)} to {round(pbfw_inc_alloc[2],1)} per 1,000 person-years."
)
