...
author:
- name: Adam Akullian,  Monisha Sharma, Jeff Imai-Eaton, Michelle O'brien, Geoff Garnett
...
```{r, echo=F, results='hide',message=FALSE, warning=FALSE, fig.cap = "Figure 1. A. Incidence rate among adults 15-24 by district. Districts with price thresholds > $130/pp/yr highlihght in red. B. Cumulative percent of all new infections occuring within a given percent of districts by incidence (arranged from high to low). 50% of all incident infections occur in 25% of the districts with the highest incidence", message=F, fig.width=15, fig.height=8}
...
ggarrange(plot_map_nnt_F1524, plot_map_pt_F1524,
                    nrow = 1,
                    #labels = c("A", "B","C","D"),
                    common.legend = F)
```
...
plot_map_inc_pt200_1524F_2 <- naomi_ssa_shp_m %>% 
  ...
  #geom_sf(data=africa_adm0_cropped_subs, color="black", fill="grey90", linewidth=0.8) +
  geom_sf(aes(fill=inc_cat),show.legend=T) +
  geom_sf(data=naomi_ssa_shp_m %>%  filter(age_group_label==age_group_selection, sex==sex_selection, pt >100, iso3 %in% iso3_selection),
    fill=NA, color="red",lwd=0.75,show.legend=F) +
  #geom_sf(data=africa_adm0_cropped_subs, color="black", fill=NA, linewidth=0.8) +
  viridis::scale_fill_viridis(option="G", discrete=T, na.value = "grey90",drop=FALSE, direction=-1) +
  facet_grid(sex~age_group_label, switch="y") +
...
districts_targeted <- filter(risk_dist_targeting,pt>130)
...
plot_price_target <- as.data.frame(naomi_ssa_shp_m) %>%
  ...
  #geom_line(aes(y = cum_pop*100),size=1,linetype=2) +
  geom_vline(xintercept=100, linetype=2) +
  scale_x_continuous(trans = "reverse", breaks=seq(0,700,50), limits=c(300,-25), expand=c(0,0)) +
  scale_color_viridis(discrete=T, na.value = "grey90",drop=FALSE) +
...
# ggarrange(plot_map_inc_pt100_1524F,plot_price_target,
#                     nrow = 1, align='h', 
#                     labels = c("A", "B","C","D"),
#                     common.legend = F)
...
```{r, echo=F, results='hide',message=FALSE, warning=FALSE,fig.cap = "Figure X. XXXX", message=F, fig.width=30, fig.height=10}
...
ggarrange(plot_map_unv_vs_target,plot_map_unv_vs_target_percentile_risk,
                    nrow = 2, 
                    labels = c("A", "B","C","D"),
                    common.legend = T)
