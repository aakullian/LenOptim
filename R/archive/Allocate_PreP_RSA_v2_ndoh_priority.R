...
allocate_prep_by_risk_with_stratified_prob <- function(facility_df,
  ...
  df_merged <- df_expanded %>%
    ...
    filter(!is.na(inc_in_sample), !is.na(catchment_population_group), sex %in% sex_allocation_selection, age_group_label %in% age_group_allocation_selection, 
           quantile_target_factor == "0.75 - 1") %>%
    mutate(allocated_units = 0) %>%
    group_by(facility_name) %>%
    mutate(
      total_catchment_15_24 = sum(
        if_else(age_group_label %in% c("15-19", "20-24"), catchment_population_group, 0),
        na.rm = TRUE
      ),
      pop_prop = if_else(
        age_group_label %in% c("15-19", "20-24"),
        catchment_population_group / total_catchment_15_24,
        1
      )
    ) %>%
    ungroup() %>%
    mutate(across(
      c(
        ...
        catchment_population_group,
        quantile_target_factor,
        catchment_population_risk_strata,
        inc_in_sample,
        inc_district,
        inc_in_sample_cat
      ), 
    ~ if_else(sex == "female" & age_group_label == "50-99", NA, .),
    .names = "{.col}"
  )) %>%
    mutate(across(
      c(
      ...
      ~ if_else(sex == "female" & age_group_label == "50-99", 46, .), #This is the estimated incidence rate of FSW in South Africa from https://www.thelancet.com/journals/lanhiv/article/PIIS2352-3018(22)00201-6/fulltext
      .names = "{.col}"
    ))
  ...
  # --- Method A: Ranked population strategy ---
    ...
    df_result <- df_result %>%
      ...
      mutate(units_needed = case_when(
        age_group_label %in% c("15-19", "20-24") ~ pop_prop * GenPop_initiations,
        age_group_label == "50-99" ~ pop_prop * KP_initiations,
        TRUE ~ NA_real_ ))#units needed is the proportion of the population in each age/sex group of the total target pop in a given district
...
format_prep_allocation_maps <- function(maps, result_df, incidence_df, efficacy, label_text = NULL) {
  ...
  p3 <- maps$district_population_choropleth +
    base_theme +
    labs(title = "District-level Len coverage")
...
generate_prep_allocation_outputs <- function(facility_df,
  ...
  # 4. Format map layout
  label_text <- paste0("Scenario: ", selected_budget/selected_cost," Len courses, ","NDOH priority districts (general allocation to women 15-24)")
