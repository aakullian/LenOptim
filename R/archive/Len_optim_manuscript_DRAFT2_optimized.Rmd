...
```{r wordcount-helpers, include=FALSE, cache=FALSE}
...
count_words_rmd <- function(path = knitr::current_input()) {
  txt <- paste(readLines(path, warn = FALSE, encoding = "UTF-8"), collapse = "\n")

  # 1. Remove YAML header
  txt <- sub("^---[\\s\\S]*?---\\s*", "", txt, perl = TRUE)

  # 2. Remove code chunks ``` ... ```
  txt <- gsub("```[\\s\\S]*?```", " ", txt, perl = TRUE)

  # 3. Remove inline code
  txt <- gsub("`r[^`]*`", " ", txt, perl = TRUE)

  # 4. Remove HTML and LaTeX math
  ...
  if (requireNamespace("stringi", quietly = TRUE)) {
    sum(stringi::stri_count_words(txt))
  } else {
...
# Whole file word count
text <- paste(readLines("Len_optim_manuscript_DRAFT2_optimized.Rmd"), collapse = " ")
...
combine_triplet <- function(df, base, fmt_mean = fmt_int, fmt_minmax = fmt_int) {
  m  <- df[[paste0("mean_", base)]]
  mi <- df[[paste0("min_",  base)]]
  ma <- df[[paste0("max_",  base)]]
  if (!is.null(mi) && !is.null(ma)) {
    paste0(fmt_mean(m), " (", fmt_minmax(mi), "–", fmt_minmax(ma), ")")
  } else {
    fmt_mean(m)
  }
}

summarise_scenario <- function(data, pt_cut, label) {
  data %>%
    filter(pt > pt_cut, age_group_label %in% c("15-24","25-34","35-49")) %>%
    group_by(sex, age_group_label) %>%
    summarise(
      n_districts             = n_distinct(area_id),
      len_coverage_mean       = sum(pop_subsample) / naomi_total_1549_ZAF$total_popatrisk,
      infections_averted_mean = sum(infections_averted) / naomi_total_1549_ZAF$total_inf,
...
# --- build scenarios ---
summ_all <- bind_rows(
  summarise_scenario(risk_dist_targeting_fine_scale,             130, "baseline @ $130 pt"),
  summarise_scenario(risk_dist_targeting_fine_scale,              40, "baseline @ $40 pt"),
  summarise_scenario(risk_dist_targeting_fine_scale_sensitivity, 130, "sensitivity @ $130 pt"),
  summarise_scenario(risk_dist_targeting_fine_scale_sensitivity,  40, "sensitivity @ $40 pt")
)
...
display_tbl <-
  ...
  transmute(
    ...
    # incidence -> 1 decimal
    incidence              = combine_triplet(., "incidence", fmt_mean = fmt_inc, fmt_minmax = fmt_inc),

    # relative incidence -> also 1 decimal
    `relative incidence`   = combine_triplet(., "inc_multiplier", fmt_mean = fmt_inc, fmt_minmax = fmt_inc),

    # integer-style with commas
    `number needed to treat` = combine_triplet(., "nnt"),
    #`cost per DALY`          = combine_triplet(., "cost_per_daly"),
    `price threshold`        = combine_triplet(., "price_threshold"),

    districts              = fmt_int(n_districts),
    `len coverage`         = percent(len_coverage_mean, accuracy = 0.1),
    `infections averted`   = percent(infections_averted_mean, accuracy = 0.1),

    # efficiency as a plain number (no percent)
    efficiency = fmt_inc(infections_averted_mean / pmax(len_coverage_mean, .Machine$double.eps))
  ) %>%
  arrange(scenario, sex, age_group_label) %>%
...
Adam Akullian<sup>1,§</sup>, Jeff Imai-Eaton<sup>2</sup>, Monisha Sharma<sup>3</sup>, Hasina Subedar<sup>4</sup>, Michelle O’Brien<sup>1</sup>, Geoff Garnett<sup>4,5</sup>
...
Figure 7 shows a sensitivity analysis comparing cost effectiveness of district-level allocation of Lenacapavir in South Africa under different assumptions on the HIV risk of those initiating Lenacapavir. When individuals using Lenacapavir have 1–2-fold greater risk of HIV acquisition than the population average, Lenacapavir is generally cost-effective across districts, and at > 2-fold greater risk it is likely cost-saving. Across the included sex–age strata (`r sum(cost_eff_numbers$scenario == "baseline @ $130 pt")`), the average cost per DALY averted was `r formatC(mean(cost_eff_numbers$mean_cost_per_daly[cost_eff_numbers$scenario == "baseline @ $130 pt"], na.rm = TRUE), format = "f", digits = 0, big.mark = ",")`, with an average NNT of `r formatC(mean(cost_eff_numbers$mean_nnt[cost_eff_numbers$scenario == "baseline @ $130 pt"], na.rm = TRUE), format = "f", digits = 1)`.  Incidence across strata averaged `r formatC(mean(cost_eff_numbers$mean_incidence[cost_eff_numbers$scenario == "baseline @ $130 pt"], na.rm = TRUE), format = "f", digits = 2)` (range `r formatC(min(cost_eff_numbers$min_incidence[cost_eff_numbers$scenario == "baseline @ $130 pt"], na.rm = TRUE), format = "f", digits = 2)`–`r formatC(max(cost_eff_numbers$max_incidence[cost_eff_numbers$scenario == "baseline @ $130 pt"], na.rm = TRUE), format = "f", digits = 2)`).  
Total infections averted were `r formatC(sum(cost_eff_numbers$sum_infections_averted[cost_eff_numbers$scenario == "baseline @ $130 pt"], na.rm = TRUE), big.mark = ",", format = "f", digits = 0)`.

The average cost per DALY averted was `r formatC(mean(cost_eff_numbers$mean_cost_per_daly[cost_eff_numbers$scenario == "baseline @ $40 pt"], na.rm = TRUE), format = "f", digits = 0, big.mark = ",")`, with an average NNT of `r formatC(mean(cost_eff_numbers$mean_nnt[cost_eff_numbers$scenario == "baseline @ $40 pt"], na.rm = TRUE), format = "f", digits = 1)`.  Incidence averaged `r formatC(mean(cost_eff_numbers$mean_incidence[cost_eff_numbers$scenario == "baseline @ $40 pt"], na.rm = TRUE), format = "f", digits = 2)` (range `r formatC(min(cost_eff_numbers$min_incidence[cost_eff_numbers$scenario == "baseline @ $40 pt"], na.rm = TRUE), format = "f", digits = 2)`–`r formatC(max(cost_eff_numbers$max_incidence[cost_eff_numbers$scenario == "baseline @ $40 pt"], na.rm = TRUE), format = "f", digits = 2)`), with `r sum(cost_eff_numbers$scenario == "baseline @ $40 pt")` strata contributing and a total of `r formatC(sum(cost_eff_numbers$sum_infections_averted[cost_eff_numbers$scenario == "baseline @ $40 pt"], na.rm = TRUE), big.mark = ",", format = "f", digits = 0)` infections averted.

Under higher-risk assumptions, the average cost per DALY averted was `r formatC(mean(cost_eff_numbers$mean_cost_per_daly[cost_eff_numbers$scenario == "sensitivity @ $130 pt"], na.rm = TRUE), format = "f", digits = 0, big.mark = ",")`, with average NNT `r formatC(mean(cost_eff_numbers$mean_nnt[cost_eff_numbers$scenario == "sensitivity @ $130 pt"], na.rm = TRUE), format = "f", digits = 1)`.  Incidence averaged `r formatC(mean(cost_eff_numbers$mean_incidence[cost_eff_numbers$scenario == "sensitivity @ $130 pt"], na.rm = TRUE), format = "f", digits = 2)` (range `r formatC(min(cost_eff_numbers$min_incidence[cost_eff_numbers$scenario == "sensitivity @ $130 pt"], na.rm = TRUE), format = "f", digits = 2)`–`r formatC(max(cost_eff_numbers$max_incidence[cost_eff_numbers$scenario == "sensitivity @ $130 pt"], na.rm = TRUE), format = "f", digits = 2)`), and total infections averted were `r formatC(sum(cost_eff_numbers$sum_infections_averted[cost_eff_numbers$scenario == "sensitivity @ $130 pt"], na.rm = TRUE), big.mark = ",", format = "f", digits = 0)`.

The average cost per DALY averted was `r formatC(mean(cost_eff_numbers$mean_cost_per_daly[cost_eff_numbers$scenario == "sensitivity @ $40 pt"], na.rm = TRUE), format = "f", digits = 0, big.mark = ",")` and the average NNT was `r formatC(mean(cost_eff_numbers$mean_nnt[cost_eff_numbers$scenario == "sensitivity @ $40 pt"], na.rm = TRUE), format = "f", digits = 1)`. Mean incidence was `r formatC(mean(cost_eff_numbers$mean_incidence[cost_eff_numbers$scenario == "sensitivity @ $40 pt"], na.rm = TRUE), format = "f", digits = 2)` (range `r formatC(min(cost_eff_numbers$min_incidence[cost_eff_numbers$scenario == "sensitivity @ $40 pt"], na.rm = TRUE), format = "f", digits = 2)`–`r formatC(max(cost_eff_numbers$max_incidence[cost_eff_numbers$scenario == "sensitivity @ $40 pt"], na.rm = TRUE), format = "f", digits = 2)`), with `r sum(cost_eff_numbers$scenario == "sensitivity @ $40 pt")` strata included and `r formatC(sum(cost_eff_numbers$sum_infections_averted[cost_eff_numbers$scenario == "sensitivity @ $40 pt"], na.rm = TRUE), big.mark = ",", format = "f", digits = 0)` infections averted.

Figure 8 displays district-level maps in South Africa showing where universal offering of Lenacapavir would be cost-effective under an average versus high (twice the district-level average) uptake assumption. Under the average uptake assumption, universal offer would only be cost effective for women under 25 years in most districts in the eastern part of the country where incidence is higher. Under the higher risk uptake assumption, many more districts are under the cost-effectiveness threshold of \<\$500/DALY. In this scenario, focusing universal offer of Lenacapavir to 20–35-year-old men and women would be a cost-effective strategy
...
param_tbl <- data.frame(
  ...
  Source    = c("", "", "", "", "", "","@donnellHeterosexualHIV1Transmission2010a")
)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, table.cap="**Table S1.** Numeric results from model runs under baseline and and using alternate costing and DALYs per HIV case paramters."}

library(knitr)

kable(display_tbl, caption = "**Table S1.** Numeric results from model runs under baseline and and using alternate costing and DALYs per HIV case paramters.", booktabs = TRUE, linesep = "", digits = 2)
```
