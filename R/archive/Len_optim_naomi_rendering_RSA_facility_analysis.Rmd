...
fig_caption: yes
title: "Geographic-informed risk prioritization of  facility-level Lenacapavir allocation"
author:
- name: Adam Akullian,  Liesl Page-Shipp, Geoff Garnett, Hasina Subedar
  affiliation: Gates Foundation, South Africa Ministry of Health
abstract: 
...
facility_sf <- st_as_sf(facilitydata_geocoded,
...
Number needed to ‘treat’ (NNT) to prevent one HIV infection:
𝑁𝑁𝑇=1⁄((𝑒.𝐼.𝑑))	
...
plot_age_dist_male <- naomi_ssa_shp_m %>% 
  ...
  filter(sex=="male", age_group_label %in%  c("15-19", "20-24", "25-34","35-49"),  iso3=="ZAF") %>%
  group_by(area_id, sex) %>%
  mutate(inc_rel_1519=incidence/incidence[age_group_label=="15-19"]) %>%
  ggplot(aes(x = age_group_label, y = inc_rel_1519,color=country)) +
  geom_point(size=2,alpha=0.5) +
...
Incidence heterogeneity by district across South Africa:
...
```{r, fig.align="center", echo=F, results='hide',message=FALSE, warning=FALSE, fig.cap = "Figure 3. Facility overlay", message=T, fig.width=15, fig.height=15}
...
#facilitydata_geocoded

st_crs(naomi_ssa_shp_m)
...
naomi_ssa_shp_m %>% 
  ...
  ggplot() + 
  geom_sf(data=africa_adm0_cropped_subs, color="black", fill=NA, linewidth=0.8) +
  geom_sf(aes(fill=inc_cat),show.legend=T, alpha=0.75) +
...
facility_district_totals <- facility_sf %>% 
  select(Correct.Facility.Name, District, Population..F.15.19, Population..F.20.24, Population..F.25.34, Population..F.35.49) %>% 
  group_by(District) %>%
  summarise(pop_F_1519 = sum(Population..F.15.19),pop_F_2024 = sum(Population..F.20.24),pop_F_2534 = sum(Population..F.25.34),pop_F_3549 = sum(Population..F.35.49))
...
risk_dist_targeting_fine_scale_p25 = risk_dist_targeting_fine_scale %>%
  ...
  stringdist_join(facility_district_totals)
...
districts_targeted <- filter(risk_dist_targeting, pt>100, iso3==iso3_selection)
...
plot_map_inc_pt200_1524F_2 <- naomi_ssa_shp_m %>% 
  ...
  #geom_sf(data=africa_adm0_cropped_subs, color="black", fill="grey90", linewidth=0.8) +
  geom_sf(aes(fill=inc_cat),show.legend=T) +
  geom_sf(data=naomi_ssa_shp_m %>%  filter(age_group_label==age_group_selection, sex==sex_selection, pt >100, iso3==iso3_selection),
    fill=NA, color="red",lwd=0.75,show.legend=F) +
  #geom_sf(data=africa_adm0_cropped_subs, color="black", fill=NA, linewidth=0.8) +
  viridis::scale_fill_viridis(option="G", discrete=T, na.value = "grey90",drop=FALSE, direction=-1) +
  facet_grid(sex~age_group_label, switch="y") +
...
boxplot1 = risk_dist_targeting_fine_scale %>%
  ...
  mutate(quantile_target_factor = paste(quant_target-0.125, "-",quant_target+0.125),
         inc_mult_group = cut(inc_mult, c(0,0.25,0.75,1.25,1.75,2.25,Inf), right=F))  %>%
  filter(age_group_label==age_group_selection, sex==sex_selection, iso3%in%iso3_selection, is.na(inc_mult_group)==F, quant_target %in% c(0.125,0.375,0.625,0.875)) %>%
...
Table1 <- naomi_ssa_shp_m_df %>%
  ...
  dplyr::select(-c(area_level_label, area_sort_order,geometry, area_level, iso3,country,calendar_quarter,region,
                   incidence, prevalence, inc_cat, nnt_cat, cd_cat,pt_cat,infections_averted_cat, incidence_reduction,
                   quarter_label, parent_area_id,nnt,cd,ci,pt,infections_averted,pop_at_risk,`HIV prevalence`,`HIV incidence`)) %>%
    arrange(area_id,sex,age_group_label)%>%
