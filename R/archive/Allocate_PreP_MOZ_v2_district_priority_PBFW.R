...
allocate_prep_by_risk_with_stratified_prob <- function(facility_df,
  ...
  # --- Method A: Ranked population strategy ---
    df_result <- df_merged %>%
      arrange(desc(inc_preg), desc(total_initiations), facility_name, sex, age_group_label) %>%
      mutate(allocated_units = 0)
    ...
    df_result <- df_result %>%
          ...
          mutate(cum_pop = cumsum(hiv_neg_preg_women)) %>%
          mutate(units_needed = ceiling(coverage_mult * hiv_neg_preg_women),
  ...
  df_result <- df_result %>%
    mutate(maternal_infections_averted = allocated_units * inc_preg / 1000 * efficacy,
           vertical_infections_averted = maternal_infections_averted * vertical_transmission_rate * vert_transmission_rate_multiplier,
           infections_averted = maternal_infections_averted + vertical_infections_averted)
...
summarize_allocation_scenarios <- function(results_list,
  ...
  summaries <- lapply(seq_along(results_list), function(i) {
    ...
    infections_averted <- sum(result$infections_averted, na.rm = TRUE) #changed to just sum infections averted to account for maternal + vertical
    ...
    avg_incidence_allocated <- weighted.mean(result$inc_preg[result$allocated_units > 0],
...
format_prep_allocation_maps <- function(maps, result_df, incidence_df, efficacy, label_text = NULL) {
  ...
  expected_infections <- sum(incidence_df$inc_district * incidence_df$pop_subsample / 1000, na.rm = TRUE)
  infections_averted <- sum(result_df$infections_averted, na.rm = TRUE)
...
generate_prep_allocation_outputs <- function(facility_df,
  ...
  # 4. Format map layout
  label_text <- paste0("Scenario: Pregnant and breastfeeding women (15-34 years) |  ", selected_budget/selected_cost," Len courses  |  ", "Uptake: ", (coverage_mult)*100," %")
  formatted_map <- format_prep_allocation_maps(
    maps = maps,
    result_df = result_df,
    incidence_df = incidence_df,
