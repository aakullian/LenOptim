...
map_prep_allocation_scenario <- function(results_list,
  ...
  district_facility_summary_nocoords_included <- facility_summary %>%
    group_by(area_id) %>%
    summarise(
      n_facilities = n(),
      n_allocated = sum(allocated),
  ...
  district_map1 <- left_join(district_shapefile, district_facility_summary_nocoords_included, by = "area_id")
...
generate_prep_allocation_outputs <- function(facility_df,
  ...
  # 
  # 4. Format map layout
  label_text <- paste0("Scenario: ", selected_budget/selected_cost," Len courses,", " Min facility capacity: " ,min_total_initiations, " initiations,", " Max age/sex PrEP coverage at facility: ", (coverage_mult*0.25)*100," %")
  formatted_map <- format_prep_allocation_maps(
    maps = maps,
    result_df = result_df,
    incidence_df = incidence_df,
