...
#join and subset (creates new 20-29 age-group)
...
naomi_total_1549_by_country <- as.data.frame(naomi_total_1549_by_country) %>%
  group_by(iso3) %>%
  summarise(total_inf_1549_bothsexes_country = sum( (Population*(1-prevalence))*incidence ),
            total_popatrisk_1549_bothsexes_country = sum(Population*(1-prevalence)))

naomi_total_1549_3countries <- naomi_total_1549_by_country %>%
  filter(iso3 %in% c("ZAF","MWI","ZWE")) %>%
  summarise(total_inf_1549_bothsexes_country = sum(total_inf_1549_bothsexes_country),
            total_popatrisk_1549_bothsexes_country = sum(total_popatrisk_1549_bothsexes_country))

total_infections_1549 <- naomi_total_1549_3countries$total_inf_1549_bothsexes_country
total_popatrisk_1549 <- naomi_total_1549_3countries$total_popatrisk_1549_bothsexes_country

naomi_total_1549_11countries <- naomi_total_1549_by_country %>%
  summarise(total_inf_1549_bothsexes_country = sum(total_inf_1549_bothsexes_country),
            total_popatrisk_1549_bothsexes_country = sum(total_popatrisk_1549_bothsexes_country))

total_infections_1549_11countries <- naomi_total_1549_11countries$total_inf_1549_bothsexes_country
total_popatrisk_1549_11countries <- naomi_total_1549_11countries$total_popatrisk_1549_bothsexes_country
...
for (i in seq(1,nrow(inc_list),)){
  for(shape in c(1)){
    for(quant in c(0,1/3,2/3)){
      ...
      sub_pop <- pop %>%
        filter(quantile>quant, quantile <quant+(1/3)) %>% #percent of population in risk group sampled (set to 1 for select all)
        summarise(
          iso3=iso3,
          area_id=area_id,
          ...
          pt=(tx+daly*cdt)/nnt,
          shape=shape, 
          quant_target=paste("[",quant,"-", quant+1/3,"]"))
      df1=sub_pop
      if(n==1){df2<-df1}else(df2<-rbind(df1,df2))
      n=2
...
for (i in seq(1,nrow(inc_list),)){
  for(shape in c(1)){
    for(quant in seq(0.125,0.875,0.05)){
        ...
        #pop$sampled_p = rescale(x^n/mean(x)) #probability of being sampled 
        sub_pop <- pop %>%
        filter(quantile>quant-0.125, quantile < quant+0.125) %>% #percent of population in risk group sampled (set to 1 for select all)
        summarise(
          sex=sex,
